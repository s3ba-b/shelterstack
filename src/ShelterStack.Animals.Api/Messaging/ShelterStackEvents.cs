...
/// <summary>
/// The wire contract this service shares with ShelterStack.Adoptions.Api over the Aspire
/// "messaging" (RabbitMQ) resource: exchange and routing-key names, the JSON settings the
/// bodies are written with, and the topology both ends declare.
/// <para>
...
public static class ShelterStackEvents
{
    ...
    /// <summary>Published by ShelterStack.Adoptions.Api when staff approve an application.</summary>
    public const string AdoptionApprovedRoutingKey = "adoption.approved";

    /// <summary>
    /// Published by this service when it cannot apply a status change an event asked for — the
    /// compensating path that keeps an asynchronous approval from failing silently.
    /// </summary>
...
/// <summary>
/// Staff approved an adoption application; the animal should move to
/// <see cref="Data.AnimalStatus.Adopted"/>.
/// <para>
...
/// <summary>
/// The status change an event asked for was refused — the animal is not in this tenant, or the
/// move is illegal per <see cref="Data.AnimalStatusTransitions"/> (a <c>MedicalHold</c> animal
/// cannot go straight to <c>Adopted</c>). Because the approval was asynchronous the caller has
/// long since had its 200, so this event is what lets ShelterStack.Adoptions.Api surface the
/// failure to staff instead of leaving the application approved for an animal that never moved.
/// </summary>
